...
## 2026-06-22（v3.7：代幣效果與敘述對齊 / next_money_extra 實作）

### 修復（代幣效果＝文字敘述）
- **「獲得X%代幣」改為「現有代幣的 X%」**（`gain_money`）：原本給小額整數，現改為 `round(現有代幣 × X%)`，標籤改為「獲得代幣＝現有代幣 X%」。
- **「代幣+X%，永久增加Y%」改為現有代幣的%、逐次遞增**（`money_escalate`）：每打出一次，取「現有代幣 ×（X% + Y%×已打次數）」；標籤「獲得代幣＝現有代幣 X%（每再打出一次 +Y%）」。實測 money=100：+10 → +19 → +31。
- **「隨機獲得+X%代幣」**亦改為現有代幣的 X%（`random_gain` 代幣分支）；讚數/鐵粉/粉絲分支維持「本卡讚數的 X%」。
- 「代幣變為兩倍/三倍」（`money_mult`）本就是現有代幣 ×2/×3，維持不變（為此次對齊的範例基準）。
- **`next_money_extra`（下一張代幣額外發動2次）實作完成**：原為無作用旗標；現會讓下一張卡的代幣效果（`gain_money`/`money_escalate`/`money_mult`）額外發動 N 次（旗標於回合結束重置）。實測 money_mult ×2 + 額外2次 = ×8。
- 全卡效果 × 文字敘述做了一次完整對照（增益＝本卡讚數%、轉換＝消耗來源資源%、代幣增益＝現有代幣%、turn_gain_bonus＝後續獲得放大），其餘 kind 已一致。

## 2026-06-22（v3.6：取消出牌張數上限 / 選牌顯示效果 / 詞條編號）

### 修改
- **取消「每回合最多 3 張」限制**：出牌只受「費用（能量）」限制，能量足夠就能一直打（`cardgame.js` `playCard` 移除 PLAY_SIZE 檢查；UI 手牌標題改為「本回合已出 N 張 · 費用剩 X」、卡片只在費用不足時變灰）。
- **選牌時顯示卡牌效果**：查看牌庫（scry）面板的每張卡現在顯示費用＋效果標籤＋消耗/保留旗標（引擎在 scry 預覽資料補上 effects/cost）；抽N丟M 的丟棄面板同樣顯示效果。
- **發文選詞條顯示編號**：詞條池與已選籃的 chip 在名稱後顯示該詞條的編號（code，如「#汗水 #健身01」），方便與配方表對照。

## 2026-06-22（v3.5：效果語意校正 / 永久增加 / 丟棄選擇 / 隨機獲得真實牌 / 編號顯示）

### 修改 / 修復
- **鐵粉/粉絲/讚數「+%」標籤明確化**：行為本就是「該卡讚數的 X%」（如 A2 鐵粉+5% = 讚數的 5%），但標籤改寫為「獲得鐵粉＝本卡讚數 5%」避免誤解；「本回合鐵粉獲得+50%」改寫為「本回合後續獲得鐵粉 +50%」（＝後續每次獲得 X 鐵粉時額外 +X×50%，本來就是乘區機制）。
- **「永久增加」→ 每打出一次遞增**（`money_escalate`）：如 D1「代幣+10%，永久增加5%」改為每打出一次本卡，代幣%再 +5%（第 1 次 10%、第 2 次 15%、第 3 次 20%…，本場累計）。引擎新增各 postId 的本場打出次數 `playCounts`。
- **「抽N丟M」可自選丟棄**（`draw_discard`）：玩家版改為暫停讓玩家點選要丟棄的手牌（pending；UI 新增丟棄選擇面板）；對手版仍自動丟最低攻擊。
- **「隨機獲得貼文」生成真實牌**（`random_post`）：改為從玩家**牌庫**中隨機抽一張符合星級（1/2/3★）的真實貼文，作為臨時卡加入手牌（含效果、讚數、費用），不再是無效果的臨時打擊卡。卡牌資料新增 `stars` 欄。
- **移除 N03a / N03b / N06 的「消耗」**。
- **打牌時卡名後顯示編號**：對戰手牌、對手揭示、查看/丟棄面板的卡片名稱後都附上貼文編號（如「溫柔（A4）」），與組牌/發文一致，方便對照配方表。

## 2026-06-22（v3.4：組牌費用顯示 / 取消輔助牌 / 發文演算法掛勾當下粉絲）

### 修改
- **組牌畫面顯示費用**：牌庫卡片左上顯示「N費」徽章（與消耗標籤），方便配牌時看費用曲線（`cardgame-ui.js` `cardEl`）。
- **取消「輔助牌」概念**：v2 起以「費用」系統取代輔助牌；`gen_from_table.py` 不再標記 support，所有牌復原為一般牌（**有讚數、會消耗出牌次數**）。F 系列（共通）變為一般 0 費牌。重生 `card-effects.js`。
- **發文演算法：顯示數據改掛勾「當下粉絲數」**（`algorithm.js` `compute`）：
  - 流量/觸及/讚數/分享等顯示數值改以帳號**當下粉絲數**（base + delta）計算 → 舊貼文也會隨帳號成長而更新，不再永遠停在發文當下的粉絲數。
  - 漲粉（followerGain）仍以「發布當下快照」計算，避免「漲粉→粉絲變多→觸及變大→更多漲粉」於 recompute 內循環爆衝（實測：成長後仍收斂，30 天僅 +2%）。
